...
function Checkout() {
  ...
  //   const db = getDatabase();
  //   set(ref(db, "checkout/" + new Date().getTime()), {
  //     username: username,
  //     usertext: usertext,
  //   });
  //   // e.prevent.default();
  ...
  return (
    <>
      ...
      >
        ...
        <div>
          <form name="form" id="formId" onSubmit={() => sendData()}>
            ...
            <label className="checkout__input-text" htmlFor="country">
              Страна
            </label>
            <input
              required
            ...
            <input type="submit" value="Заказать" className="checkout-send" />
          </form>
        </div>
      ...
      >
        ...
        <Link to="/zeon_store">
          <button className="checkout__sent-button">Продолжить покупки</button>
        </Link>
      </div>
    </>
  );
